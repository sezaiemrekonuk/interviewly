/**
 * Per-call usage accounting for the two speech calls (S04, §7.3, K13, speech spec
...
import type { SpeechCallReport } from './SpeechProvider';
import { recordLlmCall } from '../../src/lib/db';
import { logger } from '../../src/lib/logger';

export type SpeechCall = SpeechCallReport & { latencyMs: number };

const SPEECH_KIND = {
  tts: { priceKey: 'tts', unitKind: 'character' },
  stt: { priceKey: 'stt', unitKind: 'second' },
} as const;
 ...
 */
function ttsCostUsd(provider: string, characters: number): number | null {
  const price = prices.lookup(provider, SPEECH_KIND.tts.priceKey);
...
function sttCostUsd(provider: string, seconds: number): number | null {
  const price = prices.lookup(provider, SPEECH_KIND.stt.priceKey);
...
async function meter(
  interviewId: string,
  kind: keyof typeof SPEECH_KIND,
  call: SpeechCall,
  units: number,
  costUsd: number | null,
  traceId: string,
): Promise<void> {
  const { unitKind } = SPEECH_KIND[kind];
  if (costUsd === null) {
    logger.warn(
      { traceId, interviewId, provider: call.provider, kind, model: call.model },
      'PRICE_MISSING',
    );
  }
  await recordLlmCall({
...
export function meterTts(
  interviewId: string,
  characters: number,
  call: SpeechCall,
  traceId: string,
): Promise<void> {
  return meter(interviewId, 'tts', call, characters, ttsCostUsd(call.provider, characters), traceId);
}

export function meterStt(
  interviewId: string,
  seconds: number,
  call: SpeechCall,
  traceId: string,
): Promise<void> {
  return meter(interviewId, 'stt', call, seconds, sttCostUsd(call.provider, seconds), traceId);
}

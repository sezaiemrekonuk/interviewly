...
// I04 gave this step a second ring. `security.feature` and `ai_provider.feature` set up a job
// listing and nothing else, so they still drive the package seam; `profiling.feature` sets up
// a real interview first, and there the HR batch is generated by POST /profile. The presence
// of an interview id is the discriminator — the same shape `the response status is {int}`
// already uses to tell the two rings apart.
When('the HR round is generated', async function (this: AiWorld) {
  if (this.interviewId) {
    await this.generateHrRoundOverHttp();
    return;
  }

...
Last session ended: **I04 done.** `POST /interviews/:id/profile` (Zod union body — per-interview
pre-questions or a skip, `profiling`-only guard, the §3.3 two-layer profile merged and stored as
a **snapshot** with the date of birth stripped and the CV lifted into its own `cvText`), the
`profiling → hr_round` transition, and `generation.ts` (whole-round generation, the length check
`QuestionBatchSchema` deliberately does not make, all-or-nothing 1..count insertion, lazy round
creation, `HR_BATCH_REQUESTED`/`TECH_BATCH_REQUESTED`). `profiling.feature` (all four) and
`question_generation.feature` @AC-7/@AC-1 are green, which closes I03's 2-undefined-scenario gap
— the full suite is 27/27 and **CI's `acceptance` job is blocking again**.

Two spec conflicts were resolved and recorded rather than worked around. **ADR-I22:** the
technical batch is *not* generated by `POST /profile` (both acceptance scenarios forbid it,
against ADR-I07's literal wording); it is `ensureTechBatch`, an idempotent trigger **I06 must
call**. **ADR-I23:** `profiling.feature`'s two report scenarios assert I04's snapshot through
`profileVariables`, the helper I09 must also use, rather than waiting on I09's endpoint. Also:
`cucumber.js` now forces `AI_ENABLED=false` so the suite can never reach a real provider with
the live keys in `.env`, and a shared `Before` in `server.ts` clears rate-limit counters and
upserts the personas CI never seeds. See I04's `## Notes` for the full account. Not committed;
...
**I05 (CSRF/origin enforcement) and I06 (answer submission)** both became eligible when I04
landed; EXECUTE.md § 4 orders them I05 first. `POST /:id/profile` is already mounted behind
`requirePublicOrigin`, so I05 is largely an assertion task over routes that exist.

**I06 carries a live hand-off: it must call `ensureTechBatch` (ADR-I22).** The technical batch
is no longer generated by `POST /profile` — both `question_generation.feature` scenarios forbid
that — so until I06 fires the trigger, an interview reaching `tech_round` finds an empty round.
Read I04's `## Notes` → "For I06" before starting either task.

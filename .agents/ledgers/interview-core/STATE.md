...
Last session ended: **I04 done.** `profile.ts` (`POST /interviews/:id/profile`, §3.3 snapshot
merge, `profiling → hr_round`) and `generation.ts` (`generateRound`, `ensureTechBatch`,
`profileVariables`, `roundQuestionArgs`). `profiling.feature` + `question_generation.feature`
@AC-7/@AC-1 green; full suite 27/27, so I03's undefined-scenario gap is closed and CI
`acceptance` is blocking again. **ADR-I22:** tech batch is `ensureTechBatch`, not part of
`POST /profile` — **I06 must call it**. **ADR-I23:** `profileVariables` is also I09's entry
point. `cucumber.js` forces `AI_ENABLED=false`; `server.ts` `Before` clears `ratelimit:*` and
upserts personas. Details in I04 `## Notes`. Not committed.

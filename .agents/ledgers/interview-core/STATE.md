...
Last session ended: **I05 done.** Not the assertion-only task it looked like — two real
defects. **ADR-I24:** `requirePublicOrigin` now mounts once via `router.use` **above**
`router.param`, because Express runs param callbacks before route middleware, so a rejected
cross-site POST was reaching `activeInterview()`'s DB read; the middleware exempts
`GET`/`HEAD`/`OPTIONS` itself. **I06/I07/I12 mount routes plainly — never pass
`requirePublicOrigin` again.** **ADR-I25:** `interview_flow.feature` is in `cucumber.js`
`paths` with `tags: 'not @unwired'`; @AC-8/9/10/16 (I06/I07) and @AC-11 (I08) carry
`@unwired` and **the owning task deletes its own tag when it wires steps** — forget and the
scenarios silently skip. @AC-15 green; suite 28/28, 82 unit. I04's live hand-off still
stands: **I06 must call `ensureTechBatch` (ADR-I22)**. Details in I05 `## Notes`. Not
committed.
...
**I06 (answer submission, guarded advance, round handover).** I05 is done; I06 is the next
eligible row.

Two live hand-offs before you start, both in I05 `## Notes`: mount `/:id/answers` **without**
`requirePublicOrigin` (ADR-I24 — `router.use` covers it), and **delete `@unwired` from
@AC-8/@AC-9/@AC-10/@AC-16 in `interview_flow.feature`** as you wire their steps (ADR-I25),
or they skip silently. @AC-16 needs I07's `/resume` for two of its rows.

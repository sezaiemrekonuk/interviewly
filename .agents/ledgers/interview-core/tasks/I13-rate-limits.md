...
REPO: (this repo) · Depends: I03, A01 · Status: todo
...
## Steps
- [ ] **1. Write `rate-limit.ts`** — `interviewStartLimiter` (10/hr/user) and
...
- [ ] **2. Attach both** to `POST /interviews` (I03) before the handler.
- [ ] **3. Confirm the auth limiters** are attached to `/auth/register` and `/auth/login`;
  add wiring if missing (do not re-author the middleware).
- [ ] **4. Wire acceptance step-defs** for `rate_limits.feature` @AC-12 (6th interview in 24 h

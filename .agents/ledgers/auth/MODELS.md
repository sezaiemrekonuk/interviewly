...
| A04 | Building email verification: tokens, the mail job, the gate, and the two screens | `claude-opus-4.6` | Single-use hashed tokens — a new credential-bearing trust boundary |
| A05 | Building password reset: enumeration-safe request, session-revoking confirm, two screens | `claude-opus-4.6` | Account-takeover surface; no-enumeration and revoke-every-session both fail silently when done wrong |
| A06 | Building the onboarding profile: three cards, CV upload, and first-run routing | `claude-sonnet-4.6` | CRUD over one jsonb column plus three forms |

Rows A04–A06 were added 2026-08-03, from each task file's own `**Model:**` line. They were
missing, so EXECUTE.md § 5's tier check had nothing to read for three of the six tasks.

## Summary

- **`claude-opus-4.8` (2 tasks):** A01, A02
- **`claude-opus-4.6` (2 tasks):** A04, A05
- **`claude-sonnet-4.6` (2 tasks):** A03, A06

...
10. **Write your devlog:** create `.agents/devlogs/<basename of the task file>` — same
    basename, so `tasks/A01-backend-auth-module.md` pairs with
    `devlogs/A01-backend-auth-module.md`. This feeds the scored `AI_DEVLOG.md` deliverable
    and is written now, in this session, because nobody can reconstruct it later.
...
11. **Commit** as `{ID}: <title>`, e.g. `A01: Creating backend auth module`.
    Include the `.agents/ledgers/auth/` and `.agents/devlogs/` file changes in the same
    commit.

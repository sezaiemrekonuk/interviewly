Feature: LLM output schema validation
  ...
  Scenario: A malformed report never reaches the caller
    ...
    And the stub AI is configured to return a report with overall_score 101
    ...
    And the stored report overall_score is an integer in 0..100
    And every stored rounds[].score is an integer in 0..100
    And every stored questions[].score is an integer in 0..100

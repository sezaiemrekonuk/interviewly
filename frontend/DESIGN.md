...
| Search | one box per table, over the console's query language (`field:value`, `field>value`, bare words, quoted phrases). The `Syntax` panel lists the fields from the response envelope, which is the backend's own whitelist — a help panel that documents a term the server ignores is worse than none. A term the server could not understand is named back to the operator in `--warning`, never `--danger` and never silently dropped: a list that looks narrowed and is not is the one failure this box must not have |
